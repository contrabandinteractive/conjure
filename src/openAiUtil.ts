...
import { getOpenAPIKey, getOpenAPIModel, getPineconeIndexName, getPineconeEnvironment, getPineconeApiKey } from './config';
...
class OpenAiUtil {
  ...
  postChatCompletion = async (title: string, theText: string): Promise<String> => {

    const url = `https://conjurebackend.vercel.app/api/upsert`;
    const payload = {
      pineconeIndexName: getPineconeIndexName(),
      pineconeEnvironment: getPineconeEnvironment(),
      pineconeApiKey: getPineconeApiKey(),
      theOpenAIKey: getOpenAPIKey(),
      theDocTitle: title,
      theDocText: theText
    };
    const options: any = {
